...
def main():
    ...
    resume_epoch = 0
    # create model
    net = GAPNet(n_neighbor=config.pc_net.n_neighbor,num_classes=config.pc_net.num_classes)
    ...
    if config.pc_net.train.resume:
        print(f'loading pretrained model from {config.pc_net.ckpt_argfile}')
        checkpoint = torch.load(config.pc_net.ckpt_argfile)
        net.module.load_state_dict({k: v for k, v in checkpoint['model'].items()})
    ...
    for epoch in range(resume_epoch, 500):
